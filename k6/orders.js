...
function generateCustomer() {
	...
	return {
		...
		customerAddress: `${faker.address.streetName()}, ${faker.address.city()}, ${faker.address.country()}`,
	};
}
...
function generateOrderItems(productIds) {
	...
	for (let i = 0; i < numItems; i++) {
		...
		do {
			...
			usedProducts.size < productIds.length
		);
		...
		items.push({
			...
			unitPrice: parseFloat(faker.payment.price(10, 200)),
		});
	}
...
export function setup() {
	...
	throw Error("no products available to create orders");
}
...
export default function (data) {
	...
	if (!createCheck) {
		console.log(
			`Failed to create order: ${createRes.status} - ${createRes.body}`,
		);
		return;
	}
	...
	if (Math.random() > 0.9) {
		const deleteRes = http.del(`${BASE_URL}/api/orders/${orderId}`, null, {
			headers,
		});
		check(deleteRes, {
			"delete order status is 204 or 200": (r) =>
